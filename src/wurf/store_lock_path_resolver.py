...
class StoreLockPathResolver(object):
    def __init__(
        self, resolver, lock_cache_to: LockPathCache, project_path, dependency
    ):
        ...
        self.lock_cache_to: LockPathCache = lock_cache_to
    ...
    def resolve(self):
        ...
        self.lock_cache_to.add_path(dependency=self.dependency, path=lock_path)
    ...
    def __check_path(self, path):
        ...
        if not child.startswith(parent):
            raise WurfError(
                f"Locked paths must be a within parent project {parent} {child}"
            )
