...
def test_symlink_directory(testdirectory):
    ...
    try:
        create_symlink(
            from_path=foo_dir.path(), to_path=link_dir, overwrite=True, relative=True
        )
        ...
        create_symlink(
            from_path=foo_dir.path(), to_path=link_dir, overwrite=True, relative=False
        )
...
def test_symlink_file(testdirectory):
    ...
    try:
        create_symlink(
            from_path=ok_path, to_path=link_path, overwrite=True, relative=True
        )
    except RelativeSymlinkError:
        # Not all windows versions support relative symlinks - so we fallback
        # to non-realtive there
        assert sys.platform == "win32"
        create_symlink(
            from_path=ok_path, to_path=link_path, overwrite=True, relative=False
        )
